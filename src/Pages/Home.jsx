...
const Home = ({ category }) => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      const query = `${category} ${searchQuery}`.trim() || "news";

      try {
        const res = await axios.get(
          `https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&page=${currentPage}&pageSize=${pageSize}&apiKey=${API_KEY}`
        );
  ...
  return (
    <div className="home-container">
      <SearchBar onSearch={setSearchQuery} className="search-bar" />
      <h1 className="category-title">
        Search Results for "{category.charAt(0).toUpperCase() + category.slice(1)}"
      </h1>
